...
test("MessageList renders messages with parts", () => {
  ...
  // The new ToolCallMessage component should show "Modifying file" as a fallback when no path is provided
  expect(screen.getByText("Modifying file")).toBeDefined();
});

test("MessageList renders tool invocations from editor and file manager", () => {
  const messages: Message[] = [
    {
      id: "2",
      role: "assistant",
      content: "",
      parts: [
        { type: "text", text: "Working on your request" },
        {
          type: "tool-invocation",
          toolInvocation: {
            toolCallId: "call-1",
            args: { command: "create", path: "/Component.jsx" },
            toolName: "str_replace_editor",
            state: "result",
            result: "Success",
          },
        },
        {
          ...
          toolInvocation: {
            toolCallId: "call-2",
            args: { command: "rename", path: "/Component.jsx", new_path: "/ComponentRenamed.jsx" },
  ...
  expect(screen.getByText("Renaming /Component.jsx to /ComponentRenamed.jsx")).toBeDefined();
});
